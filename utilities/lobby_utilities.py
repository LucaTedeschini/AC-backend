...
def first_available_lobby(manager, member_id):
    ...
    if allLobbies_response.status_code == 200:
        ...
        Logger.get_logger("lobby_utilities").error(f"Failed to fetch lobbies: {allLobbies_response.text}")
    return None
...
def add_member_to_lobby(manager, lobby_id, member_id):
    ...
    if add_member_response.status_code != 200:
        Logger.get_logger("lobby_utilities").error(f"Failed to add member {member_id} to lobby {lobby_id}: {add_member_response.text}")
        return None
    ...
    if updated_lobby.status_code == 200:
        ...
        Logger.get_logger("lobby_utilities").error(f"Failed to update lobby: {updated_lobby.text}")
        return None
...
def quit_lobby(manager, lobby_id, member_id):
    ...
    if member_id not in currentMembersIds:
        Logger.get_logger("lobby_utilities").info(f"Member {member_id} is not in the lobby {lobby_id}")
        return lobby_data
    ...
    if remove_member_response.status_code != 200:
        Logger.get_logger("lobby_utilities").error(f"Failed to remove member {member_id} from lobby {lobby_id}: {remove_member_response.text}")
        return None
    updated_lobby = manager.make_api_request(
        requests.get,
        f"api/collections/lobbies/{lobby_id}?relations=members"
